...
import com.carota.htmltext.span.NumberSpan;
...
class HtmlTagHandler implements Html.TagHandler {
    ...
    private void startDelete(Editable output) {
        int len = output.length();
        output.setSpan(new Style(), len, len, Spannable.SPAN_MARK_MARK);
    }

    private void endDelete(Editable output) {
        int len = output.length();
        Object obj = getLast(output, Style.class);
        int start = output.getSpanStart(obj);
        output.removeSpan(obj);
        output.delete(start, len);
    }
