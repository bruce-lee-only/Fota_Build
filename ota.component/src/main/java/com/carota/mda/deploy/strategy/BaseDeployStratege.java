...
public abstract class BaseDeployStratege implements DeviceUpdater.IEventListener,IDeployStratege {
    ...
    protected final void addFakeLogicReport(String name, int errorCode, boolean secure, int target, boolean rollback) {
        ...
        if (SlaveInstallResult.STEP_NONE == errorCode || SlaveInstallResult.STEP_DEPLOY == errorCode) {
            ...
            mDataFactory.saveLogStateEventV2(name, state, state, rollback ? FotaState.OTA.ROLLBACK.CODE_INSTALL_VERIFY_MD5 : FotaState.OTA.UPGRADE.CODE_INSTALL_VERIFY_MD5);
            mDataFactory.saveLogStateEventV2(name, state, state, rollback ? FotaState.OTA.ROLLBACK.CODE_INSTALL_VERIFY_PKI : FotaState.OTA.UPGRADE.CODE_INSTALL_VERIFY_PKI);
        }
    }
