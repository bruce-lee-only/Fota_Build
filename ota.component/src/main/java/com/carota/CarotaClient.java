...
import com.carota.build.ParamConfig;
...
public class CarotaClient {
    ...
    public static synchronized ISession check(Bundle extra, ICheckCallback callback) throws ExecutionException {
        if(waitBootComplete("CK")) {
            ...
            return sCore.check(extra, callback);
        } else {
            return null;
        }
    }
    ...
    /**
     * @param rootDir 存放文件的根路径
     * @param keys 要获取config文件的key {@link com.carota.core.remote.IActionMDA}getConfigDownloadUrl keys
     * @return Map<String, String>,第一个参数是key，第二个参数是对应的文件路径
     */
    public static Map<String, String> getConfigFiles(String rootDir, List<String> keys) {
        waitMainCtrlReady("G-CONFIG");
        return sCore.getConfigFiles(rootDir, keys);
    }

    private static void initConfigFile(Context context) {
        Logger.info("CLIENT initConfigFile");
        ParamConfig mParamConfig = ConfigHelper.get(context).get(ParamConfig.class);
        if (mParamConfig != null && mParamConfig.isEnabled()) {
            String mHost = mParamConfig.getHost();
            String vin = CarotaVehicle.queryVehicleDetail(1).getVinCode();
            File fileDir = new File(context.getFilesDir(), "configuration");
            ConfigManager.init(fileDir, mHost);
            ConfigManager.syncPreConfig(mParamConfig.getConfigUrl(), vin, mParamConfig.getMaxRetry());
        }
    }
}
