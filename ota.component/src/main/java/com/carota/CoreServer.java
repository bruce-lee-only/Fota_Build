...
import com.carota.config.provider.ConfigManagerService;
...
public class CoreServer {
    ...
    private HubService initServer(Context ctx, Map<String, RouterService> funcMap,
        ...
        RouterService rs;
        if (paramDM.isEnabled()) {
            rs = new DownloadManagerService(ctx, paramDM.getHost(),paramDM.getDownloadDir(ctx),paramDM.getRetry(),paramDM.getReserveSpace(),paramDM.getLimitSpeed());
            funcMap.put(rs.toString(), rs);
            Logger.debug("rpc-svr : INIT-DM @%s",paramDM.getHost());
        }
        ...
        if (paramHttpProxy.isEnabled()) {
            rs = new HttpProxyService(paramHttpProxy.getHost(), paramHttpProxy.getWhiteList());
            funcMap.put(rs.toString(), rs);
            Logger.debug("rpc-svr : INIT-HPS");
        }
        ParamConfig paramConfig = cfg.get(ParamConfig.class);
        if(paramConfig.isEnabled()) {
            rs = new ConfigManagerService(ctx, paramConfig.getHost());
            funcMap.put(rs.toString(), rs);
            Logger.debug("rpc-svr : INIT-CMS");
        }
