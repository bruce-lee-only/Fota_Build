...
public class ActionMDA implements IActionMDA {
    ...
    @Override
    public Map<String, String> getConfigFiles(String rootDir, List<String> keys) {
        final String CALL_TAG = LogUtil.TAG_RPC_MDA + "[GET-CONFIG] ";
        try {
            MasterDownloadAgent.ConfigReq.Builder builder = MasterDownloadAgent.ConfigReq.newBuilder()
                    .addAllConfigNames(keys)
                    .setTag(ReqTag.TAG_SRC_MDA);
            PrivReqHelper.Response response = PrivReqHelper.doPost(mBaseUrl + "/config", builder.build().toByteArray());
            Logger.info(CALL_TAG + "RSP : %1d", response.getStatusCode());
            if (PrivStatusCode.OK.equals(response.getStatusCode())) {
                Map<String, String> configFiles = new HashMap<>();
                MasterDownloadAgent.ConfigRsp rsp = MasterDownloadAgent.ConfigRsp.parseFrom(response.getBody());
                File dir = new File(rootDir, "Config");
                FileHelper.mkdir(dir);
                FileHelper.cleanDir(dir);
                for (MasterDownloadAgent.ConfigRsp.ConfigUrl configUrl : rsp.getConfigUrlsList()) {
                    Logger.info(CALL_TAG + "config = " + configUrl.getConfigName() + " / url = " + configUrl.getUrl());
                    File config = new File(dir, configUrl.getConfigName());
                    File tmp = new File(dir, configUrl.getConfigName() + ".tmp");
                    boolean download = PrivReqHelper.doDownload(configUrl.getUrl(), tmp);
                    Logger.info(CALL_TAG + "download = " + download);
                    if (download && tmp.renameTo(config)) {
                        configFiles.put(configUrl.getConfigName(), config.getPath());
                    }
                }
                return configFiles;
            }
